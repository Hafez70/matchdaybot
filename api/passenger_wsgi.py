...
request_log = "/home/fcfun/fifa-bot/api/REQUEST_LOG.txt"

with open(marker_file, "w") as f:
    f.write(f"Passenger started at: {__import__('datetime').datetime.now()}\n")
    f.write(f"Python: {sys.executable}\n")
    f.write(f"SUCCESS: App loaded!\n")
...
def application(environ, start_response):
    """WSGI application wrapper with logging"""
    import datetime
    ...
    try:
        result = _asgi_app(environ, start_response)
        with open(request_log, "a") as f:
            f.write(f"  -> OK\n")
        return result
